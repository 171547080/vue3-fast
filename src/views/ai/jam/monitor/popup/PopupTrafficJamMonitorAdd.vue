<template>
    <gc-modalx v-model:show="cfg.show" v-model:submitting="cfg.submitting" v-model:cancelText="cfg.cancelText"
        ...
        <gc-formx ref="formx" :formState="formState">
            ...
            <a-form-item required has-feedback label="设备类型" name="type">
            ...
            <a-form-item v-if="formState.type" required has-feedback :label="deviceTypeMap[formState.type] + '地址'"
                name="dataSource"
            ...
            <a-form-item required has-feedback label="经度" name="longitude"
                :rules="[{ required: true, validator: $vaild.queue([$vaild.numberBetween(-180, 180), $vaild.required]) }]">
                <a-input v-model:value="formState.longitude" autocomplete="off" :disabled="cfg.submitting" />
            </a-form-item>
            <a-form-item required has-feedback label="纬度" name="latitude"
                :rules="[{ required: true, validator: $vaild.queue([$vaild.numberBetween(-90, 90), $vaild.required]) }]">
                <a-input v-model:value="formState.latitude" autocomplete="off" :disabled="cfg.submitting" />
            </a-form-item>
            <a-form-item has-feedback label="方向" name="direction"
                :rules="[{ validator: $vaild.queue([$vaild.maxLength(2, 50)]) }]">
