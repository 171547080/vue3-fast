    ...
    >
      ...
      <div>
        <a-tooltip v-for="d in themes" :key="d.color" placement="top">
          ...
          <div  class="themes-color-item" :style="{backgroundColor: d.color}" @click="clickThemeColor(d)">
              <check-outlined v-if="currentThemeColor === d.color"/>
            </div>
        </a-tooltip>
      </div>
      ...
      <div class="custom-themes-warp">
        ...
        <template v-if="customThemes">
          <a-input
            v-model:value="currentThemeColor"
            autocomplete="off"
      ...
      <div class="cfg-item">
        <p>反色模式</p>
        <p><a-switch v-model:checked="inverseColorMode" @click="()=>switchInverseColorMode"/></p>
      </div>
      <a-divider/>
...
// 监听反色模式
watch(inverseColorMode, (newvalue) => {
  const inverColorClassName = 'color-weak'
  const appDom = document.getElementById('app')
  const mainDom = appDom && appDom.firstElementChild ? appDom.firstElementChild : null

  if (mainDom && newvalue && mainDom.className.indexOf(inverColorClassName) <= -1) {
...
// 当前用户设置的主题颜色
const currentThemeColor = ref<string>(appStore.themeColor)
onBeforeMount(() => {
  currentThemeColor.value = appStore.themeColor
  switchThemeColor({ color: currentThemeColor.value })
})
...
const switchThemeColor = (theme?) => {
  ...
  appStore.themeColorChange(currentThemeColor.value)
}
</script>
<style lang="less">
/**
...
.custom-class .ant-drawer-content {
  overflow: visible;
}
.custom-class .ant-drawer-body {
  overflow: visible;
}
.custom-class .open-btn-warp{
  position: absolute;
  z-index: 999;
  left: -48px;
  top: 280px;
}
.custom-class .themes-color-item{
  width: 25px;
  height: 25px;
  font-size: 17px;
...
.custom-class .custom-themes-warp{
  padding: 20px 0;
}
</style>
